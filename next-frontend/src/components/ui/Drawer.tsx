...
export function Drawer({ isOpen, onClose, title, children, side = "right" }: { isOpen: boolean, onClose: () => void, title?: string, children: React.ReactNode, side?: "left" | "right" }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex bg-black/60 backdrop-blur-sm">
      ...
      <div className={`fixed top-0 bottom-0 w-full max-w-md bg-card border-border shadow-2xl flex flex-col duration-300 ${sideClasses} ${side === "right" ? "border-l" : "border-r"}`}>
        <div className="flex items-center justify-between p-6 border-b border-border bg-black/20">
          {title && <h2 className="text-xl font-semibold">{title}</h2>}
          <button onClick={onClose} className="p-2 hover:bg-white/10 rounded-md transition-colors text-muted-foreground hover:text-white">
            <X size={20} />
          </button>
        </div>
